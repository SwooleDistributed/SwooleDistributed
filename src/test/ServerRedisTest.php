...
class ServerRedisTest extends TestCase
{
    ...
    {
        $this->redis_pool->getCoroutine()->del(["testSet2", "testSet1", "test", "test1", "key1", "testSet", "testlist1"
    ...
    {
        $value = $this->redis_pool->getCoroutine()->ping();
        if (!$value) {
    ...
    {
        $value = $this->redis_pool->getCoroutine()->set('test', 'testRedis');
        if (!$value) {
    ...
    {
        $value = $this->redis_pool->getCoroutine()->setex('test', 10, 'testRedis');
        if (!$value) {
    ...
    {
        $this->redis_pool->getCoroutine()->setnx('test', 'testRedis2');
    ...
    {
        $this->redis_pool->getCoroutine()->set('test', 'testRedis');
        $this->redis_pool->getCoroutine()->rename('test', 'test1');
        $value = $this->redis_pool->getCoroutine()->get('test1');
    ...
    {
        $value = $this->redis_pool->getCoroutine()->mset(array('key0' => 'value0', 'key1' => 'value1'));
        if (!$value) {
            throw new SwooleTestException('redis mset 失败');
        }
        $value = $this->redis_pool->getCoroutine()->get('key0');
        if ($value != 'value0') {
            throw new SwooleTestException('redis mset 失败');
        }
        $value = $this->redis_pool->getCoroutine()->get('key1');
        if ($value != 'value1') {
            throw new SwooleTestException('redis mset 失败');
        }
    }
    ...
    {
        $value = $this->redis_pool->getCoroutine()->mget(['key0', 'key1', 'key2']);
    ...
    {
        $this->redis_pool->getCoroutine()->del('test');
        $value = $this->redis_pool->getCoroutine()->get('test');
        if ($value != null) {
            throw new SwooleTestException('redis del 失败');
        }
        $this->redis_pool->getCoroutine()->del(array('key0', 'key1'));
        if ($value != null) {
            throw new SwooleTestException('redis del 失败');
        }
        $value = $this->redis_pool->getCoroutine()->get('key1');
    ...
    {
        $this->redis_pool->getCoroutine()->set('key1', 0);
        $this->redis_pool->getCoroutine()->incr('key1');
        $value = $this->redis_pool->getCoroutine()->get('key1');
        if ($value != 1) {
    ...
    {
        $this->redis_pool->getCoroutine()->set('key1', 0);
        $this->redis_pool->getCoroutine()->incrBy('key1', 10);
        $value = $this->redis_pool->getCoroutine()->get('key1');
        if ($value != 10) {
    ...
    {
        $this->redis_pool->getCoroutine()->set('key1', 1);
        $this->redis_pool->getCoroutine()->decr('key1');
        $value = $this->redis_pool->getCoroutine()->get('key1');
        if ($value != 0) {
    ...
    {
        $this->redis_pool->getCoroutine()->set('key1', 10);
        $this->redis_pool->getCoroutine()->decrBy('key1', 10);
        $value = $this->redis_pool->getCoroutine()->get('key1');
        if ($value != 0) {
            throw new SwooleTestException('redis incrBy 失败');
        }
    ...
    {
        $value = $this->redis_pool->getCoroutine()->exists('key1000');
        if ($value) {
            throw new SwooleTestException('redis exists 失败');
        }
        $value = $this->redis_pool->getCoroutine()->exists('key1');
        if (!$value) {
            throw new SwooleTestException('redis exists 失败');
        }
    }
    ...
    {
        $value = $this->redis_pool->getCoroutine()->lpush('testlist', 'test1');
        if (!$value) {
    ...
    {
        $value = $this->redis_pool->getCoroutine()->rpush('testlist', 'test2');
        if (!$value) {
    ...
    {
        $value = $this->redis_pool->getCoroutine()->lpop('testlist');
        if ($value != 'test1') {
    ...
    {
        $value = $this->redis_pool->getCoroutine()->rpop('testlist');
        if ($value != 'test2') {
    ...
    {
        $this->redis_pool->getCoroutine()->lpush('testlist', 'test1');
        $value = $this->redis_pool->getCoroutine()->lset('testlist', 0, 'test0');
        if (!$value) {
    ...
    {
        $value = $this->redis_pool->getCoroutine()->lIndex('testlist', 0);
        if ($value != 'test0') {
    ...
    {
        $value = $this->redis_pool->getCoroutine()->lLen('testlist');
        if ($value != 1) {
    ...
    {
        $this->redis_pool->getCoroutine()->lpush('testlist', 'test3');
        $value = $this->redis_pool->getCoroutine()->lRange('testlist', 0, -1);
        if (count($value) != 2) {
    ...
    {
        $this->redis_pool->getCoroutine()->lpush('testlist', 'test4');
        $this->redis_pool->getCoroutine()->lpush('testlist', 'test5');
        $this->redis_pool->getCoroutine()->lpush('testlist', 'test6');
        $value = $this->redis_pool->getCoroutine()->lTrim('testlist', 0, 1);
        if (!$value) {
    ...
    {
        $this->redis_pool->getCoroutine()->lpush('testlist', 'testrem');
        $this->redis_pool->getCoroutine()->lpush('testlist', 'testrem');
        $value = $this->redis_pool->getCoroutine()->lRem('testlist', 'testrem', 0);
        if (!$value) {
    ...
    {
        $this->redis_pool->getCoroutine()->lset('testlist', 0, 'testinsert');
        $this->redis_pool->getCoroutine()->lInsert('testlist', \Redis::AFTER, 'testinsert', 'testinsert1');
        $value = $this->redis_pool->getCoroutine()->lIndex('testlist', '1');
    ...
    {
        $this->redis_pool->getCoroutine()->rpush('testlist', 'testrpoplpush');
        $this->redis_pool->getCoroutine()->rpoplpush('testlist', 'testlist1');
        $value = $this->redis_pool->getCoroutine()->lIndex('testlist1', '0');
    ...
    {
        $value = $this->redis_pool->getCoroutine()->sAdd('testSet', 'index0');
    ...
    {
        $value = $this->redis_pool->getCoroutine()->sIsMember('testSet', 'index0');
    ...
    {
        $value = $this->redis_pool->getCoroutine()->sRem('testSet', 'index0');
    ...
    {
        $this->redis_pool->getCoroutine()->sAdd('testSet', 'index0');
        $this->redis_pool->getCoroutine()->sAdd('testSet', 'index1');
        $value = $this->redis_pool->getCoroutine()->sCard('testSet');
    ...
    {
        $value = $this->redis_pool->getCoroutine()->sRandMember('testSet');
    ...
    {
        $value = $this->redis_pool->getCoroutine()->sPop('testSet');
    ...
    {
        $this->redis_pool->getCoroutine()->del(['testSet', 'testSet1']);
        $this->redis_pool->getCoroutine()->sAdd('testSet', 'index0');
        $value = $this->redis_pool->getCoroutine()->sMove('testSet', 'testSet1', 'index0');
        $this->assertTrue($value, 'redis sMove 失败');
        $value = $this->redis_pool->getCoroutine()->sPop('testSet1');
    ...
    public function testRedisSInter()
    {
        ...
        $this->redis_pool->getCoroutine()->sAdd('testSet', 'index0');
        $this->redis_pool->getCoroutine()->sAdd('testSet', 'index1');
        $this->redis_pool->getCoroutine()->sAdd('testSet1', 'index0');
        $this->redis_pool->getCoroutine()->sAdd('testSet1', 'index1');
        $value = $this->redis_pool->getCoroutine()->sInter('testSet1', 'testSet');
    ...
    public function testRedisSInterStore()
    {
        ...
        $this->redis_pool->getCoroutine()->sAdd('testSet1', 'index0');
        $this->redis_pool->getCoroutine()->sAdd('testSet1', 'index1');
        $this->redis_pool->getCoroutine()->sInterStore('testSet2', 'testSet1', 'testSet');
        $value = $this->redis_pool->getCoroutine()->scard('testSet2');
    ...
    public function testRedisSUnion()
    {
        ...
        $this->redis_pool->getCoroutine()->sAdd('testSet', 'index0');
        $this->redis_pool->getCoroutine()->sAdd('testSet', 'index1');
        $this->redis_pool->getCoroutine()->sAdd('testSet1', 'index2');
        $this->redis_pool->getCoroutine()->sAdd('testSet1', 'index3');
        $value = $this->redis_pool->getCoroutine()->sUnion('testSet1', 'testSet');
    ...
    public function testRedisSUnionStore()
    {
        ...
        $this->redis_pool->getCoroutine()->sAdd('testSet1', 'index2');
        $this->redis_pool->getCoroutine()->sAdd('testSet1', 'index3');
        $this->redis_pool->getCoroutine()->sUnionStore('testSet2', 'testSet1', 'testSet');
        $value = $this->redis_pool->getCoroutine()->scard('testSet2');
    ...
    {
        $this->redis_pool->getCoroutine()->del('testSet');
        $this->redis_pool->getCoroutine()->sAdd('testSet', 'index0');
        $this->redis_pool->getCoroutine()->sAdd('testSet', 'index1');
        $value = $this->redis_pool->getCoroutine()->sMembers('testSet');
    ...
    {
        $this->redis_pool->getCoroutine()->del('testuid');
        $this->redis_pool->getCoroutine()->lpush('testuid', 1);
        $this->redis_pool->getCoroutine()->set('testuid_name_1', 'test1');
        $this->redis_pool->getCoroutine()->set('testuid_level_1', 10);
        $this->redis_pool->getCoroutine()->lpush('testuid', 2);
        $this->redis_pool->getCoroutine()->set('testuid_name_2', 'test2');
        $this->redis_pool->getCoroutine()->set('testuid_level_2', 20);
        $value = $this->redis_pool->getCoroutine()->sort('testuid', ['sort' => 'desc']);
        $this->assertCount(2, $value, 'redis Sort 失败');
        $value = $this->redis_pool->getCoroutine()->sort('testuid', ['by' => 'testuid_level_*', 'sort' => 'desc']);
        $this->assertCount(2, $value, 'redis Sort 失败');
        $value = $this->redis_pool->getCoroutine()->sort('testuid', ['by' => 'testuid_level_*', 'get' => ['testuid_name_*', '#', 'testuid_level_*'], 'sort' => 'desc']);
    ...
    {
        $this->redis_pool->getCoroutine()->set('test', 42);
        $value = $this->redis_pool->getCoroutine()->getSet('test', 'lol');
    ...
    {
        $this->redis_pool->getCoroutine()->set('test', 'test');
        $this->redis_pool->getCoroutine()->append('test', 'lol');
        $value = $this->redis_pool->getCoroutine()->get('test');
    ...
    {
        $this->redis_pool->getCoroutine()->set('test', 'test');
        $value = $this->redis_pool->getCoroutine()->strlen('test');
    ...
    {
        $this->redis_pool->getCoroutine()->hset('testHash', 'key0', 'value0');
        $value = $this->redis_pool->getCoroutine()->keys('testHash');
    ...
    {
        $value = $this->redis_pool->getCoroutine()->hget('testHash', 'key0');
    ...
    {
        $value = $this->redis_pool->getCoroutine()->hlen('testHash');
    ...
    {
        $this->redis_pool->getCoroutine()->hdel('testHash', 'key0');
        $value = $this->redis_pool->getCoroutine()->hlen('testHash');
    ...
    {
        $this->redis_pool->getCoroutine()->hset('testHash', 'key0', 'value0');
        $value = $this->redis_pool->getCoroutine()->hkeys('testHash');
    ...
    {
        $value = $this->redis_pool->getCoroutine()->hvals('testHash');
    ...
    {
        $value = $this->redis_pool->getCoroutine()->hGetAll('testHash');
    ...
    {
        $value = $this->redis_pool->getCoroutine()->hExists('testHash', 'key0');
    ...
    {
        $this->redis_pool->getCoroutine()->hset('testHash', 'key1', 1);
        $this->redis_pool->getCoroutine()->hIncrBy('testHash', 'key1', 10);
        $value = $this->redis_pool->getCoroutine()->hget('testHash', 'key1');
    ...
    {
        $this->redis_pool->getCoroutine()->hMset('testHash', ['key1' => 1, 'key2' => 2]);
        $value = $this->redis_pool->getCoroutine()->hget('testHash', 'key1');
        $this->assertEquals($value, 1, 'redis hMset 失败');
        $value = $this->redis_pool->getCoroutine()->hget('testHash', 'key2');
    ...
    {
        $value = $this->redis_pool->getCoroutine()->hMget('testHash', ['key1', 'key2']);
    ...
    public function testRedisZAdd()
    {
        ...
        $value = $this->redis_pool->getCoroutine()->keys('testZset');
    ...
    {
        $value = $this->redis_pool->getCoroutine()->zRange('testZset', 0, -1);
        ...
        $value = $this->redis_pool->getCoroutine()->zRange('testZset', 0, -1, true);
    ...
    {
        $value = $this->redis_pool->getCoroutine()->zRevRange('testZset', 0, -1);
        ...
        $value = $this->redis_pool->getCoroutine()->zRevRange('testZset', 0, -1, true);
    ...
    {
        $this->redis_pool->getCoroutine()->del('testZset');
        $this->redis_pool->getCoroutine()->zadd('testZset', 0, 'vol0');
        $this->redis_pool->getCoroutine()->zadd('testZset', 5, 'vol5');
        $this->redis_pool->getCoroutine()->zadd('testZset', 2, 'vol2');
        $value = $this->redis_pool->getCoroutine()->zCount('testZset', 0, 5);
    ...
    public function testRedisZCard()
    {
        ...
        $this->redis_pool->getCoroutine()->zadd('testZset', 5, 'vol5');
        $this->redis_pool->getCoroutine()->zadd('testZset', 2, 'vol2');
        $value = $this->redis_pool->getCoroutine()->zCard('testZset');
    ...
    {
        $value = $this->redis_pool->getCoroutine()->zRem('testZset', 'vol0');
    ...
    {
        $this->redis_pool->getCoroutine()->zadd('testZset', 0, 'vol0');
        $value = $this->redis_pool->getCoroutine()->zScore('testZset', 'vol0');
    ...
    public function testRedisZRank()
    {
        ...
        $this->redis_pool->getCoroutine()->zadd('testZset', 5, 'vol5');
        $this->redis_pool->getCoroutine()->zadd('testZset', 2, 'vol2');
        $value = $this->redis_pool->getCoroutine()->zRank('testZset', 'vol2');
    ...
    public function testRedisZRevRank()
    {
        ...
        $this->redis_pool->getCoroutine()->zadd('testZset', 5, 'vol5');
        $this->redis_pool->getCoroutine()->zadd('testZset', 2, 'vol2');
        $value = $this->redis_pool->getCoroutine()->zRevRank('testZset', 'vol2');
    ...
    {
        $this->redis_pool->getCoroutine()->zadd('testZset', 2, 'vol2');
        $this->redis_pool->getCoroutine()->zIncrBy('testZset', 10, 'vol2');
        $value = $this->redis_pool->getCoroutine()->zScore('testZset', 'vol2');
    ...
    {
        $this->redis_pool->getCoroutine()->del('testZset');
        $this->redis_pool->getCoroutine()->zadd('testZset', 0, 'vol0', 5, 'vol5', 2, 'vol2');
        $this->redis_pool->getCoroutine()->zRemRangeByScore('testZset', 0, 5);
        $value = $this->redis_pool->getCoroutine()->zCard('testZset');
    ...
    public function testRedisZUnion()
    {
        ...
        $this->redis_pool->getCoroutine()->del('testZset1');
        $this->redis_pool->getCoroutine()->zadd('testZset1', 1, 'vol1');
        $this->redis_pool->getCoroutine()->zadd('testZset1', 3, 'vol3');
        $this->redis_pool->getCoroutine()->zadd('testZset1', 4, 'vol4');

        $this->redis_pool->getCoroutine()->zunion('testZset2', ['testZset', 'testZset1'], [1, 2], 'SUM');
        $value = $this->redis_pool->getCoroutine()->zRange('testZset2', 0, -1);
    ...
    public function testRedisZInter()
    {
        $this->redis_pool->getCoroutine()->del('testZset');
        $this->redis_pool->getCoroutine()->zadd('testZset', 0, 'vol0');
        $this->redis_pool->getCoroutine()->zadd('testZset', 5, 'vol3');
        $this->redis_pool->getCoroutine()->zadd('testZset', 2, 'vol4');
        ...
        $this->redis_pool->getCoroutine()->zInter('testZset2', ['testZset', 'testZset1'], [1, 2], 'SUM');
        $value = $this->redis_pool->getCoroutine()->zRange('testZset2', 0, -1);
